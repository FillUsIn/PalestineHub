...
async function createUserAccount(createUserDTO: CreateUserDTO): Promise<Post> {
  const response = await api.post<Post>(`${API_BASE_URI}`, createUserDTO, { withCredentials: true });
  return response.data;
}

async function login(loginDTO: LoginDTO): Promise<Post> {
    const response = await api.post<Post>(`${API_BASE_URI}/login`, loginDTO, { withCredentials: true });
    return response.data;
}

async function logout(): Promise<Post> {
    const response = await api.post<Post>(`${API_BASE_URI}/logout`, { withCredentials: true });
    return response.data;
}
